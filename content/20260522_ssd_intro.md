...
Beyond just "competitive math", we can also talk a bit about what the tasks actually look like. Here's an example from [BeyondAIME](https://huggingface.co/datasets/ByteDance-Seed/BeyondAIME), which is one of the eval sets we'll use:
...
So that's the goal for this project: see if there is an SSD-like effect on competitive math when running it on Qwen3.5-4B, and see if it does result in improved performance. I will also implement my own stack and not reuse the implementation from [official repo](https://github.com/apple/ml-ssd), because that sounds like a fun thing to learn how to do. In the next post I'll talk about what the data for this project will look like.
